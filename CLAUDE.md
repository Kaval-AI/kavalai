...
| `examples/` | Runnable usage examples; `green_village/` is the evaluation example — the RAG chatbot served on port 25000 by `green_village_support_in_memory.py`, with its 64 cases in `eval_cases.yaml` (the older `eval/` directory belongs to the removed suite format); `chat_client/` is the terminal chat client for any agent server speaking `user_message` → `agent_response` (other reply fields shown as truncated JSON; tested by `tests/test_chat_client.py`); `business_info_agent.py` also seeds the agent DB with demo runs (agent, sessions, runs, tasks, chat, model stats) for backoffice demos and screenshots |
