...
| `examples/` | Runnable usage examples; `green_village/` is the RAG evaluation example — the chatbot served on port 25000 by `green_village_support_in_memory.py`, with its 64 cases in `eval_cases.yaml`; `bakery/` is the YAML-workflow example — `assistant.yaml` is the graph, `bakery.py` the catalogue/models/order book/tools it calls, and `bakery_in_memory.py` (port 25100, SQLite + a list) / `bakery_real_db.py` (port 25101, Postgres for both the agent DB and the order book) differ only in which database they hand it, with 29 cases in `eval_cases.yaml`; `chat_client/` is the terminal chat client for any agent server speaking `user_message` → `agent_response` (other reply fields shown as truncated JSON; tested by `tests/test_chat_client.py`); `business_info_agent.py` also seeds the agent DB with demo runs (agent, sessions, runs, tasks, chat, model stats) for backoffice demos and screenshots |
...
- `examples/bakery/eval_cases.yaml` is the side-effecting one: 29 cases against
