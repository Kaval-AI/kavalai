...
| `tools/` (repository root, distinct from `kavalai/tools/`) | Standalone product tooling that is not part of the wheel; `websitescraper/` scrapes a website into a pages database (`scrape.py` CLI via `python -m tools.websitescraper.scrape`, `pages_db.py` the resumable crawl state, `htmlmd.py` stdlib HTML→markdown) — the first half of the site-chatbot pipeline planned in `chatbotplan.md`; its tests live in its `tests/` subfolder |
