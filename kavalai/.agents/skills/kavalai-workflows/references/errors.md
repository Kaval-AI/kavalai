...
| `… branch 'a' reaches the start node 'x'.` | A branch transitions back to `start` | Point the branch at the join node instead. |
| `… branches 'a' and 'b' share node(s) […]. Branch subgraphs must be disjoint.` | One node reachable from two branches | Give each branch its own nodes; share work after the join. |
| `… branches 'a' and 'b' both write […]. Each branch must write its own output variable.` | Two branches store to one `output` | Use a distinct `output` per branch. |
...
- A `switch` with no matching case and no `default` raises `WorkflowException`
  (`Workflow halted at node 'x' with no next node and without reaching an end
  node.`).
