...
Each release opens with a summary. **Added**, **Changed**, **Removed** and
**Fixed** list the changes to the package. **Upgrading** lists what an existing
...
Every run records how long it took, the backoffice opens a project's database
read-only, and retrieval can be filtered by metadata. The release carries one
migration in each set.
...
* **Run duration.** ``runs.duration_seconds`` (agents revision 0006) records
  ...
  ``duration_seconds``, and ``AgentService.update_run`` takes it.

  * The backoffice shows it on each run of a conversation. The daily runtime
    chart sums it, falling back to ``updated_at - created_at`` for runs
    recorded before the column existed, which are not backfilled. The
    workflows page lays runs out in lanes by when they actually ended.
...
* **Read-only projects in the backoffice.** ``projects.read_only`` (backoffice
  ...
  query_only`` on SQLite.

  * ``DatabaseManager.get_sessionmaker(read_only=True)`` and
    ``SqliteRagService(read_only=True)`` are new, and the RAG service the
    explorer builds no longer provisions on any project.
  * ``projects.rag_schema`` names the schema of a project's RAG collections
    when a deployment keeps them apart from the runtime tables; empty means
    the agent schema, as before.
...
* **Metadata filtering in retrieval.** ``query`` and ``query_batch`` take
  ...
  equal — the condition ``delete_by_metadata`` already deleted by.
  ``batch_query_with_join`` takes it too.

  * The filter is a condition of the nearest-neighbour search itself, not
    applied to its result, so ``top_k`` matching entries come back when the
    collection holds that many: on PostgreSQL it is ``metadata @> :match``
    under the collection's GIN index, with pgvector's iterative index scan
    enabled as it is for ``source_ids``; on SQLite it joins the ``WHERE``
    clause of the full scan.
  * The ``rag_query`` node and ``WorkflowBuilder.rag_query`` carry it as
    ``match``. A string value is a template, and one that is exactly one
    ``{{ context.* }}`` placeholder takes the referenced value with its type,
    so a filter an earlier node extracted passes through unchanged. The
    rendered filter is recorded with the node's inputs on its task row.
...
A release about serving an agent to the public. Outbound requests are guarded
against server-side request forgery, the stream a visitor sees can be filtered,
and a turn's spend is bounded. The two RAG services share one storage model, a
SQLite file is accepted wherever a database URI is, the chat widget ships in
the wheel, and ``common`` is divided into four extras by weight. Test doubles
(:mod:`kavalai.testing`) and an HTML parser and chunker (:mod:`kavalai.text`)
join the base install.
...
see, or to spend without bound. This section names them; the detail is under
the headings that follow.

* **Server-side request forgery.** ``http_request`` and ``crawl_url`` refuse
  loopback, private, link-local and cloud metadata targets. ``http_request``
  connects to the address it checked, so DNS rebinding cannot redirect it.
  An agent meant to reach an intranet is built with
  ``allow_private_networks=True``, an argument of the tool factory that the
  model cannot set. See :mod:`kavalai.net` under *Serving and tools*.
...
* **Unbounded spend per turn.** ``max_output_tokens`` caps what a call may
  generate, ``history_limit`` and ``history_max_chars`` bound the history an
  ``llm`` node sends, and ``run_timeout`` bounds a run. A misspelt cap in
  ``llm_kwargs`` fails the load instead of being ignored. ``get_chat_history``
  returned the oldest fifty messages of a session, so a long conversation paid
  for a full history on every turn while the model never saw the latest
  exchange.
...
* **Least privilege.** ``provision=False`` on the RAG services issues no DDL,
  ...
  connects as needs no ``CREATE`` right on the RAG schema.
* **Personal data in the agent database.** ``record_payloads=False`` keeps
  ...
  the session deletes blank the model-call payloads of the conversations they
  remove, so a deleted conversation does not survive as a prompt in
  ``model_call_stats``.
...
* ``sqlite:///path`` is accepted wherever a database URI is: ``KAVALAI_DB_URI``
  ...
  and :func:`~kavalai.rag.rag_service_from_uri`, the counterpart of
  ``PostgresRagService.from_uri`` that picks the RAG service from the URI
  scheme and forwards further keyword options to it.
...
* ``provision=False`` on the RAG services: no DDL; a missing registry reads as
  ...
  ``create_collection()``. Collections are created beforehand with
  ``create_collection(name, dim, *, model=None, vector_type=None)``, which
  issues DDL whatever ``provision`` says, and ``ensure_registry()`` is public.
...
* ``model_call_stats.session_id`` and ``run_id`` (agents migration ``0005``):
  ...
  per conversation is a query.

  * ``TaskLogger.log_model_call``, ``TokenAccumulator``, ``StatsBridge`` and
    ``AgentService.add_model_call_stats`` take ``session_id`` and ``run_id``;
    ``get_model_call_stats`` filters by ``agent_id``, ``session_id`` and
    ``run_id``.
  * ``MemoryTaskLogger.model_calls`` holds ``ModelCallRecord`` objects
    carrying the ids, with ``model_calls_for_run``; ``SqliteTaskLogger``'s own
...
* :mod:`kavalai.net`, a guard against server-side request forgery for tools
  ...
  and :func:`~kavalai.net.ensure_public_url` refuse loopback, private,
  link-local, carrier-grade NAT, multicast, reserved and cloud metadata
  targets, and any IPv6 address that maps, translates or tunnels to such an
  IPv4 address. :class:`~kavalai.net.PublicOnlyTransport` is an httpx
  transport that resolves a host once, checks every address and connects to
  the address it checked, so DNS rebinding cannot redirect it; each redirect
  hop is checked again. ``make_http_request(allow_private_networks=True)`` and
...
* The chat widget ships in the wheel as ``kavalai.widget``
  ...
  * ``texts`` for every visible string, aria labels and error messages
    included, and a disclosure label, ``AI assistant``, on by default
    (``disclosure: false`` removes it).
  * ``onFeedback(runId, vote, comment)`` with thumbs and an optional comment
    box, and ``widget.on(...)`` events (``open``, ``close``, ``reply``,
    ``error``, ``resize``, ``feedback``).
...
* ``GET /projects/{project_id}/llm-call-stats`` accepts ``agent_id``,
  ``session_id`` and ``run_id``. The **Model calls** page links each call to
  its conversation and to its run's tasks, filters by the ``run_id`` and
  ``session_id`` query parameters, and is reached from a conversation's
  **Model calls** button. The task debugger lists the run's model calls —
  tokens and duration per call — beneath its tasks.
* ``GET /projects/{project_id}/rag/collections`` returns each collection's
  name, model, dimension, schema version and entry count.

Packaging, testing and text
"""""""""""""""""""""""""""
...
Workflow engine and LLM clients
"""""""""""""""""""""""""""""""
...
* The conversation list is ordered by last activity — the time of a session's
  most recent run — and labels its timestamp accordingly. It no longer depends
  on Postgres-only SQL (``DISTINCT ON``, ``jsonb_typeof``):
  :func:`~kavalai.db.json_typeof` and :func:`~kavalai.db.json_array_length`
  render per dialect and a window function ranks the messages.
...
Third-party providers register by name, the workflow engine gains ``rag_query``
and ``parallel`` nodes and an explicit lifecycle, and :mod:`kavalai.eval`
grades a running agent. The packaging extras and several environment variables
are renamed, so read *Changed* and *Removed* before upgrading.
...
Streaming execution, the Anthropic client and a SQLite RAG service, on a
flattened package layout with Alembic migrations.
